...
function Login({ isLoginPage }) {
  const [loginOpen, setLoginOpen] = useState(isLoginPage ? true : false);
    ...
    reader.addEventListener("load", async (event) => {
      ...
      fetch(uploadUrl, {
        ...
        .then((res) => {
          ...
          console.log("res при загрузке файла = ", res);
          if (res.status === "success") {
            setUploadAlarmObj({
              open: true,
              severity: "success",
    ...
    <>
      ...
      <Dialog fullScreen={fullScreen} open={loginOpen} onClose={handleClose}>
        ...
        <DialogActions>
          ...
          <LoadingButton loading={popupLoader} onClick={handleSubmit}>
            <span>Войти</span>
          </LoadingButton>
        </DialogActions>
      </Dialog>

      <Dialog fullScreen={fullScreen} open={uploadOpen} onClose={handleClose}>
        <DialogTitle>Загрузить в общюю библиотеку</DialogTitle>
        <DialogContent>
          <DialogContentText>
            <Collapse in={uploadAlarmObj.open}>
              <Alert severity={uploadAlarmObj.severity}>
                {uploadAlarmObj.message}
              </Alert>
            </Collapse>
          </DialogContentText>
        </DialogContent>
          ...
          <Stack alignItems="flex-end">
            ...
            >
              ...
              <VisuallyHiddenInput
                ...
                onChange={handleAddEntity.bind(null, "back")}
              />
            </LoadingButton>
            <LoadingButton
              component="label"
              loading={imageButtonLoader}
              startIcon={<LocalFloristIcon />}
            >
              Добавить изображение
              <VisuallyHiddenInput
                inputProps={{ accept: ".png, .jpeg" }}
                onChange={handleAddEntity.bind(null, "image")}
              />
            </LoadingButton>
          </Stack>
        </DialogActions>
